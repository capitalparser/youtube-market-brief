...
| Analyze | `claude` CLI subprocess 호출 → 영상별 요약·핵심 인사이트·레드팀 분석·종목 영향 | `src/.../pipeline/analyze.py` |
...
- 모든 외부 의존(YouTube API, youtube-transcript-api, claude CLI, Telegram Bot API)은 `_clients/`로 격리.
...
- Capture Depth 결정: `watchlist_hits` 비면 `light`, 있으면 `deep` (`00_Wiki/CLAUDE.md` 매핑). frontmatter 필드 `tier` 값으로 소문자 기록.
...
- 멱등성: 동일 video_id 재투입 시 skip (`state/store.py`)
...
- LLM: `claude` CLI subprocess (`claude -p --output-format json`). Anthropic API 키 미사용.
...
- Whisper ASR fallback (transcript 비공개 영상 자동 전사)
