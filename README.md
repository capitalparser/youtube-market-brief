...
```
   YouTube Data API ─────► Discover (신규 video_id 추출)
                              │
   yt-dlp + cookies ◄─── Transcribe (자막 fetch, 클라우드 IP 우회)
                              │
...
| `ymb aggregate-only --date YYYY-MM-DD [--no-telegram] [--full-body]` | vault에 이미 생성된 영상 MD들을 모아 brief만 재구성·발송 |
...
YouTube는 GitHub Actions의 클라우드 IP를 봇으로 차단한다. `yt-dlp` + 쿠키 인증으로만 cloud에서 transcript 수집이 가능하며, 쿠키는 **수 주 ~ 몇 달 단위**로 만료된다. 다음 에러가 다시 보이면 갱신 필요:
